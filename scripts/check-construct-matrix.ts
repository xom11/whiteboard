...
//
// Logic kiểm tra tách thành `runMatrixCheck()` (pure, không side-effect) để Jest
// import được làm CI guard (Phase 6b). CLI side-effect (in bảng + process.exit)
// gói trong `main()` — CHỈ chạy khi invoke trực tiếp (`require.main === module`),
// KHÔNG chạy khi import từ test.
//
...
/**
 * Verify CONSTRUCT_MANIFEST khớp registry thật (pure — không in, không exit).
 * Trả về { errors, warnings }. `errors.length === 0` ⇒ matrix hợp lệ.
 */
export function runMatrixCheck(): MatrixCheckResult {
...
/** CLI: in bảng + WARN/ERRORS, exit≠0 nếu có error. */
function main(): void {
  const { errors, warnings } = runMatrixCheck();
...
// CLI side-effect chỉ khi chạy trực tiếp (`npx tsx scripts/check-construct-matrix.ts`),
// KHÔNG khi Jest import { runMatrixCheck }.
