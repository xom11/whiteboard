...
- **✅ FIX 2026-06-11 (halo highlight điểm phái sinh):** `JxgRenderer.addHalo` (viền xám khi chọn object trong editor) trước switch theo `el.elType` string CHỈ có `point/glider/intersection` → điểm phái sinh `circumcenter`(O)/`otherintersection`(M,N,P)/`perpendicularpoint`(D,E,F)/`incenter` KHÔNG được halo dù click VẪN chọn được (selection dùng `objKind` ưu tiên `elementClass`). FIX: addHalo nhận diện point-like qua `elementClass===1` (OBJECT_CLASS_POINT, JSXGraph set cho MỌI subtype điểm) — đồng bộ với `objKind` (tools.tsx). **GOTCHA: chọn-object (objKind) dùng `elementClass` (robust) NHƯNG halo (addHalo) trước dùng `elType` string (thiếu) → không nhất quán; bất kỳ logic phân-loại-element nào nên ưu tiên `elementClass`.**
