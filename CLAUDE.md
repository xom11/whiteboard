...
- **Dataset `T02_problems.txt` (2026-06-10): 5→10/76 (~13%, olympiad).** Probe `npx tsx scripts/diag-t02.ts [-v]`. 90% KHÔNG khả thi tầng này (chuỗi giao-của-giao, điểm subscript A1/A2, điểm phẩy A'B'C', quỹ tích, dựng-hình, định lý thuần, radical axis/mixtilinear/harmonic). Thêm: `intersection` DISTRIB_PAIRS "giao của các cặp (AC,BD);(AB,CD)" (toàn-đề, blob `[^.]+`) + `∩`; `tangentsAtMeet` (2 tiếp tuyến tại B,C cắt nhau); `triangle` "ABC là tam giác vuông tại A" (nhớ thêm vào `patterns[]`); onSegment/incircleTangency/perpFoot nới separator "và" + "đồng quy". Gotcha: rule `.match()` khớp nhưng escalate → check PREFILTER `patterns[]` chặn rule chạy HOẶC vocab thiếu keyword.
