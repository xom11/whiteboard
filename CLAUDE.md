...
- **Kiến trúc pipeline (single pipeline, rules-first — đại tu 2026-06-09):**
  ...
  - **Coverage scan 2026-06-09** (probe 22 đề thực tế, deterministic-only): 14→**16/22 HIT**. MISS còn lại (TARGET tiếp, escalate LLM an toàn): (1) **parallel/perp line through point** "Qua A kẻ đường thẳng song song BC" — CHƯA có rule (intent `draw-line` parallelThrough/perpThrough đã có, chỉ thiếu NLU); (2) `angleBisectorFoot` phrasing "chân đường phân giác từ A"; (3) perpBisector ∩ line "trung trực BC cắt AB tại D" (intersection-of-derived-line, khó); (4) "đường tròn (O), dây AB" (circle no-radius + chord); (5) arcMidpoint implied circumcircle khi KHÔNG nêu (O). Tái tạo scan: probe `tryDeterministicFigure` (xem git commit msg).
