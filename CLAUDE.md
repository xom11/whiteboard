...
- **Kiến trúc pipeline (single pipeline, rules-first — đại tu 2026-06-09):**
  ...
  - **✅ excenter ĐÃ PORT (2026-06-09)**: rule `ai/rules/excenter.ts` ("J là tâm bàng tiếp góc A" → excenter, of=[A,B,C], opposite). Priority 60, VN-only.
  - **Coverage gaps — TARGET PORT TIẾP** (hiện escalate LLM an toàn, chưa có rule deterministic): `arcMidpoint` cần circle context tường minh trong phrasing; excircle ("vẽ đường tròn bàng tiếp") chưa có intent path (chỉ có excenter point). `_shared.ts extractPointName` NAME_LA cũng dính bug `là\b` (rule mới nên tự neo tên, đừng phụ thuộc) — cân nhắc fix khi rảnh.
