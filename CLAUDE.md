...
- **Kiến trúc pipeline (single pipeline, rules-first — đại tu 2026-06-09):**
  ...
  - **Coverage scan 2026-06-09** (probe 22 đề thực tế, deterministic-only): 14→**17/22 HIT** (+ "nội tiếp (O)", distributive "lần lượt", parallel/perp-line). MISS còn lại (TARGET tiếp, escalate LLM an toàn): (1) `angleBisectorFoot` phrasing "D là chân đường phân giác từ A" (cevian xử lý "phân giác AD" nhưng chưa dạng này); (2) perpBisector ∩ line "trung trực BC cắt AB tại D" (intersection-of-derived-line, khó); (3) "đường tròn (O), dây AB" (circle no-radius + chord); (4) arcMidpoint implied circumcircle khi KHÔNG nêu (O); (5) intersection "giao điểm của X và Y" generic. Tái tạo scan: probe `tryDeterministicFigure`.
