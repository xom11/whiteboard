...
- **Thêm mức độ dày nét = đặt số thẳng, KHÔNG có API**: Excalidraw 0.18 khoá cứng `STROKE_WIDTH = {thin:1, bold:2, extraBold:4}` (`dist/dev/chunk-4FTI6OG3.js:366`) và `actionChangeStrokeWidth` render đúng 3 nút đó — `UIOptions` không có chỗ thêm mức. Nhưng `strokeWidth` chỉ là một con số trên element/appState, **không bị clamp ở đâu**, nên `updateScene` với 0.25 là hợp lệ. Giải pháp ở `src/ui/StrokeWidthSlider.tsx` + `strokeWidth.ts` (hàm thuần) + `useStrokeWidth.ts`: portal 1 `<input type=range>` vào CHÍNH `<fieldset>` của Excalidraw (neo bằng `[data-testid="strokeWidth-thin"]`, testid này **sống sót qua bản prod** — verify: `dist/prod/index.js` có đúng 1 lần), rồi ẩn `.buttonList` gốc bằng CSS. Gắn vào TRONG fieldset là cố ý: fieldset chỉ tồn tại khi tool/selection có nét, nên slider ăn theo luôn logic hiện/ẩn của Excalidraw. Thang 0.25→4 bước 0.25 (luỹ thừa 2 ⇒ không có rác dấu phẩy động; 3 mức cũ rơi đúng nấc nên bảng cũ không đổi hình). **Vì sao cần**: bút vẽ tay nhân `strokeWidth * 4.25` (`getFreeDrawSvgPath`, `chunk-4FTI6OG3.js:9320`) nên "thin"=1 ra nét ~4,25px trong khi shape cùng giá trị chỉ 1px — đó là lý do GV kêu "nét nhỏ nhất vẫn đậm".
  - **⚠️ BẪY ĐÃ TRẢ GIÁ — grep selector CSS bị cắt tiền tố**: `grep -o "\.buttonList[^}]*}"` trả `.buttonList{...display:flex...}` làm tưởng specificity 1 lớp, nên viết `.excalidraw .wb-stroke-width-fieldset > .buttonList` (0,3,0) là đủ thắng. Selector THẬT là `.excalidraw .panelColumn .buttonList` — cũng (0,3,0) ⇒ **hoà, rồi thua theo thứ tự nạp**, hàng 3 nút vẫn hiện nguyên. Legend y hệt: `.excalidraw .panelColumn legend` (0,2,1). Phải **nhân đôi class** (`.wb-stroke-width-fieldset.wb-stroke-width-fieldset`), cùng mẹo với `.App-menu__left.App-menu__left`. jsdom không có cascade nên **unit test xanh hết**; chỉ e2e bắt được. Khi đọc CSS đã minify, grep phải lấy CẢ selector (`grep -o "[^{}]*buttonList[^{}]*{[^}]*}"`).
  - `currentItemStrokeWidth` được thêm vào `SyncableAppState` — Excalidraw không coi đó là dữ liệu scene, nhưng không lưu thì GV chọn nét mảnh xong F5 là về mặc định 2.
  - **Coupling class/testid nội bộ** (`.buttonList`, `legend`, `strokeWidth-thin`) → bump 0.19 phải chạy `npx playwright test tests/e2e/stroke-width-slider.spec.ts` (có ca đếm pixel mực trên canvas thật, đã kiểm chứng âm: ép `STROKE_WIDTH_MIN=1` thì 3 ca đỏ).
